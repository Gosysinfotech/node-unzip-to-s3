...
internals._validateOptions = function (options) {
  ...
  if (options.hasOwnProperty('filter') && typeof(options.filter) != 'function')
...
internals.streamWithOptions = function (options) {
  ...
  return function (zipStream) {
    ...
    var stream = internals.unzipFiles(zipStream)
    if (options.filter)
      stream = stream.pipe(options.filter);
    stream.pipe(internals.uploadFiles(client, fileStream, options.dirPath));
